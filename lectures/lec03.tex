...
Erinnerung (Diff 2): Sei $ M \subset \R^n $ eine Untermannigfaltigkeit, $p \in M$. Ein Tangentialvektor $v \in \R^n$ an $M$ in $p$ ist von der Form $ v = \gamma'(0) $, wobei $ \gamma: (-\epsilon,\epsilon) \to M $ mit $\gamma(0) = p$ eine in $M$ verlaufende $C^1$-Kurve ist. Der \emph{Tangentialraum} $T_aM$ (an $M$ in $p$) ist die Menge aller Tangentialvektoren. 
\begin{itemize}
...
\begin{exmp*}
	$ \mathbb{S}^2 \subset \R^3,\ T_pM = p^\bot $, denn $ f: \R^n \to \R, f(x) = 1-\|x\|^2 $ für $ p \in \S^2 $, sodass $p_i > 0$ oder $<0$ beschreibt $ \mathbb{S}^2 \cap U_i \ni p, U_i = \{x \in \R^3 \mid x_i > 0 $ oder $ < 0\} $ als Nullstellengebilde und $ Df(p) = -2p. $
	\image{2.png}{14cm}
\end{exmp*}
...
\begin{defn}[Keime]\index{Keime}
	...
	$$ f \sim g \iff \existss \text{offene Umgebung } U \ \text{von } p \text{, sodass } \bound{f}{V} = \bound{g}{V} $$
	eine Äquivalenzrelation. Eine Äquivalenzklasse bezüglich $\sim$ nennt man \emph{Keim} einer Abbildung $M \to N$ bei $p$. Ist $ f $ $C^1$/glatt bei $p$, so auch alle Elemente der von $f$ repräsentierten Klasse $\bar{f}$. Man spricht in dem Fall von $C^1$/glatten Keimen. Wir schreiben $ \E^1(p) $ bzw. $\E^\infty(p)$ für die Menge aller $C^1$ bzw. glatten Keime bei $p \in M$ mit Werten in $\R$ ("Funktionskeime") $ (M,p) \to \R $.
\end{defn}

\begin{rem}
	\begin{enumerate}[label={\roman*})]
		\item $\E^1(p)$ und $E^\infty(p)$ bilden eine Algebra (denn die Bildung von Äquivalenzklassen ist mit der punktweisen Addition und Multiplikation verträglich).
		\item Ist $ \bar{f}: (M,p)\to N $ ein $C^1$/glatter Keim bei $p \in M$, so definiert er einen Homomorphismus von Algebren \big($\E(f(p))$ Keime $(N,f(p)) \to \R$\big)
		\[ f^*: \E^{1/\infty} (f(p)) \to E^{1/\infty} (p) \ \text{vermöge } f^* \left( \bar{h} \right) = \bar{h} \circ \bar{f} \qquad \text{"pullback", "Zurückziehung"} \]
		Offensichtlich ist das unabhängig von der Wahl des Repräsentanten.
		\image{2_2.png}{14cm}
		Es gilt $\id^* = \id$ und $(f \circ g)^* = g^* \circ f^*$. Insbesondere induziert ein bezüglich Komposition integrierbarer Keim $\bar{f}$ ($ \bar{f} \circ \bar{f}^{-1} = \bar{f} \circ \overbar{f^{-1}} = \id $) einen Isomorphismus $f^*, f^{-1 *} \circ f^* = \id$.
		\item Spezialfall: Ist $\varphi$ eine um $p \in M$ zentrierte Karte, so definiert $\varphi$ einen invertierbaren Keim $ \overbar{\varphi}: (M,p) \to \R^n $ mit $\varphi(p)=0$ und somit einen Isomorphismus $ \varphi^*: \E^{1/\infty}_n \to \E^{1/\infty} (p) $, wobei $ \E_n = \{$Funktionskeime bei $0 \}, $ das heißt man kann sich auf Keime aus $\E_n$ beschränken.
	\end{enumerate}
\end{rem}
...
\begin{defn}[algebraische Definition]\index{Tangentialraum}\index{Derivation}
	Eine \emph{Derivation} von $\E^\infty(p)$ ist eine lineare Abbildung $X: \E^\infty(p) \to \R$, die der folgenden Leibniz-Regel genügt:
	\[ X(\bar{f} \cdot \bar{g}) = X(\bar{f}) \cdot g(p) + f(p) \cdot X(\bar{g}). \]
	Der \emph{Tangentialraum} $T_pM$ in $p$ ist der Vektorraum der Derivationen von $\E^\infty(p)$.\\
	Ein glatter Keim $ \bar{f}: (M,p) \to N $ induziert einen Algebra-Homomorphismus $ f^*: \E^\infty (f(p)) \to \E^\infty (p) $ und somit eine (lineare) Abbildung, die \emph{Tangentialabbildung} (oder Differential) $ T_pf: T_pM \to T_{f(p)}N, X \mapsto X \circ f^*. $
\end{defn}
...
\begin{rem*}
	\begin{enumerate}[label={\roman*})]
		...
		denn für $\bar{h} \in \E^\infty (g(f(p)))$ gilt
		\begin{align*}
		...
		\end{align*}
		\item Ist $\bar{\varphi}: (M,p) \to \R^n$ ein Keim einer bei $p$ zentrierten Karte, so ist $ \E_n^\infty \overset{\cong}{\underset{\varphi^*}{\longrightarrow}} \E^\infty(p) $ isomorph und $ T_pM \overset{\cong}{\underset{T_p \varphi}{\longrightarrow}} T_0\R^n $. $T_0\R^n$ hat eine besonders einfache Beschreibung:
	\end{enumerate}
\end{rem*}
...
\begin{thm}
	Seien $ (x_1,\dotsc,x_n),(y_1,\dotsc,y_n) $ lokale Koordinaten bei $p \in M, q \in N$, gegeben durch bei $p$ bzw. $q$ zentrierte Karten $ \varphi,\psi $ (das heißt $ x_i(\tilde{p}) = \varphi_i (\tilde{p}) $ mit $(\varphi,U)$ lokale Karte bei $p \in M, \tilde{p} \in Um \varphi(p) = 0$. Analog für $y_j$.)\\
	Dann sind $ \{ \bound{\frac{\del}{\del x_1}}{0}, \dotsc, \bound{\frac{\del}{\del x_n}}{0} \} $ bzw. $ \{ \bound{\frac{\del}{\del y_1}}{0}, \dotsc, \bound{\frac{\del}{\del y_n}}{0} \} $ Basen von $ R_0\R^m \cong T_pM $ bzw. $ T_0\R^n \cong T_qM $.\\
...
\begin{exmp*}
	\begin{minipage}{\linewidth}
		\begin{wrapfigure}{R}{0.3\textwidth}
			\centering
			\includegraphics[width=0.1\textwidth]{2_5_1.png}
		\end{wrapfigure}
		$ M = \mathbb{S}^1, p = (0,1,0)^T, \begin{aligned}[t]
			&\varphi: U_2^+ \cap \mathbb{S}^2 \to \R^2, \\&\varphi(x) = (x_1,x_3)
		\end{aligned} $
	\end{minipage}
		
		Basis des Tangentialraums $ T_0\R^2 $ $ \bound{\del_1}{0},\bound{\del_2}{0} $\\
		$ f_{1,2}: \mathbb{S}^2 \to \mathbb{S}^2, \begin{aligned}[t]
			&f_1(x) = (x_1,-x_2,x_3),\\ &f_2(x) = (x_1,x_2,-x_3)
		\end{aligned} $\\
		$ \overbar{g_1} = \psi_1 \circ \overbar{f_1} \circ \varphi^{-1}: (\R^2,0) \to \R^2,\ \psi_1: U_2^- \cap \mathbb{S}^2 \to \R^2, \psi_1 (x_1,x_2,x_3) = (x_1,x_3) $\\
...
\subsection*{Verhalten unter Kartenwechseln}
	...
	Seien $ \bar{\varphi},\bar{\psi}: (M,p) \to (\R^n,0) $ Kartenkeime. Dann ist der Kartenwechsel $ \Phi = \bar{\psi} \circ \bar{\varphi}^{-1}: (\R^m,0) \to \R^m $ ein glatter invertierbarer Keim und $\Phi$ ist eindeutig durch $ \psi,\varphi $ festgelegt. Solche $\Phi$ bilden eine Gruppe $\G$ (bezüglich Komposition $\circ$). Durch die Zuordnung $ \Phi \mapsto D\Phi(0) $ erhält man einen Gruppenhomomorphismus $ \G \to GL(m,\R) $.
...
\begin{defn}[geometrische Definition]\index{Tangentialraum}
	...
	\[ \overbar{\gamma_1} \sim \overbar{\gamma_2} \iff \frac{d}{d t}\left( \bar{f} \circ \overbar{\gamma_1} \right)(0) = \frac{d}{d t}\left( \bar{f} \circ \overbar{\gamma_2} \right)(0) \qquad \foralll \bar{f} \in \E^\infty(p). \]
...
\begin{rem*}
	...
	Sei $ \bar{f} : (M,p) \to N $ ein glatter Keim, dann ist $ \underbrace{[\gamma]}_{\in (T_pM)_{geom}} \mapsto \underbrace{[f \circ \gamma]}_{\in (T_{f(p)} N)_{geom}} $ die Tangentialabbildung $T_pf$, denn $\begin{aligned}[t]
		 ...
		 &= T_pf(X_\gamma)(\bar{h}) 
	\end{aligned} \ \foralll \bar{h} \in \E^\infty_{f(p)} $
	\image{2_8 rem}{12cm}
\end{rem*}
