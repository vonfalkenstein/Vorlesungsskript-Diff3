	...
	\begin{itemize}
		\item lokal euklidisch:\\
			...
			\begin{rem*}
				...
				\[ \begin{tikzcd}
					\tilde{U} \arrow[bend left=50]{rrr}{\varphi} & & & \varphi\big(\tilde{U}\big) \arrow[bend left=50]{lll}{\varphi^{-1}}
				\end{tikzcd} \]
...
\begin{exmp}
	\begin{enumerate}[label= {\roman*})]
		...
		\item $\Sbb^n := \{ x \in \R^{n+1} \mid \|x\|_E = 1 \} $
...
\begin{defn}[Differenzierbare Struktur]\index{Mannigfaltigkeit!Differenzierbare Struktur}
	Sei $\Acal$ ein differenzierbarer Atlas (von $M$), dann bezeichnet man mit $\Dcal = \Dcal(\Acal)$ die Menge \emph{aller} Karten von $M$, die mit allen Karten aus $\Acal$ glatte Kartenwechsel haben,
